#!/usr/bin/env bash
# Configuration
WALLPAPER_DIR="$HOME/Imagens"  # Change this to your wallpaper directory
...
if [ -n "$selected" ]; then
    ...
    if [ -n "$original_path" ]; then
        # Set wallpaper and regenerate colors from the selected image.
        "$HOME/.config/hypr/apply-wallpaper-theme.sh" "$original_path"
